...
# These are the searches we run on the Kent Police news search page
SEARCH_TERMS = [
    "homophobic",
    "transphobic",
    "biphobic",
...
# Used for tagging and relevance checks
LGBT_CORE_TERMS = [
...
def is_relevant_lgbt_hate(text_lower: str) -> bool:
    ...
    # Weaker signal: hate crime + LGBT mention together
...
def parse_kent_police_article(url: str) -> dict | None:
    ...
    published = ""
    m = re.search(
        r"Published:\s*([0-9]{2}:[0-9]{2}\s[0-9]{2}/[0-9]{2}/[0-9]{4})",
        text_all,
    )
    if m:
        published = m.group(1)
